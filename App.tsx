...
const SKILLS = [
  ...
  { id: "EF12EF06", text: "(EF12EF06) Discutir a importância de observação das normas e das regras dos esportes de marca e de precisão." },
...
const FIXED_EVALUATION_HEADER: string[] = [];

const FIXED_ADAPTATION_HEADER: string[] = [];
...
const App: React.FC = () => {
  ...
  const toggleStrategy = (rowId: string, strategy: string) => {
    const row = data.rows.find(r => r.id === rowId);
    if (!row) return;
    let current = row.estrategias;
    const formattedStrategy = `- ${strategy}`;
    if (current.includes(formattedStrategy)) {
      current = current.replace(formattedStrategy, '').replace(/\n\n+/g, '\n').trim();
    } else {
      current = current ? `${formattedStrategy}\n\n${current}` : formattedStrategy;
    }
    handleRowChange(rowId, 'estrategias', current);
  };

  const toggleEvaluation = (rowId: string, evaluation: string) => {
    const row = data.rows.find(r => r.id === rowId);
    if (!row) return;
    let current = row.avaliacao;
    const formattedEval = `- ${evaluation}`;
    if (current.includes(formattedEval)) {
      current = current.replace(formattedEval, '').replace(/\n\n+/g, '\n').trim();
    } else {
      current = current ? `${current}\n${formattedEval}` : formattedEval;
    }
    handleRowChange(rowId, 'avaliacao', current);
  };

  const toggleAdaptation = (rowId: string, adaptation: string) => {
    const row = data.rows.find(r => r.id === rowId);
    if (!row) return;
    let current = row.adaptacao;
    const formattedAdapt = `- ${adaptation}`;
    if (current.includes(formattedAdapt)) {
      current = current.replace(formattedAdapt, '').replace(/\n\n+/g, '\n').trim();
    } else {
      current = current ? `${current}\n\n${formattedAdapt}` : formattedAdapt;
    }
    handleRowChange(rowId, 'adaptacao', current);
  };
            ...
            <div key={row.id} className="bg-white border-2 border-slate-200 rounded-2xl overflow-hidden shadow-lg group">
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-3">
                  ...
                  <textarea value={row.estrategias} onChange={(e) => handleRowChange(row.id, 'estrategias', e.target.value)} className="w-full h-[240px] p-3 border-2 rounded-lg text-[11px]" />
                </div>
                ...
                <td>
                  <div className="space-y-2">
                    {FIXED_METHODOLOGY_HEADER.map((t, i) => (
                      <p key={i} className="font-bold mt-2">{t}</p>
                    ))}
